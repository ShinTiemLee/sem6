/**
 *	Reverse each word of string in parallel
 *
 *	"EHT ERIPME DID GNIHTON GNORW" -> "THE EMPIRE DID NOTHING WRONG"
 */
...
int main(int argc, char **argv)
{
    ...
    cl_mem mem_indices; // device memory used for space indices;

    int i = 0;

    // Ain't no body got time to take input :p
    char *inpooot = (char *)malloc(100 * sizeof(char));
    ...
    FILE *kernel_code_file = fopen("kern.cl", "r");
    ...
    global_work_size = spaces; // That many work items should be there
    local_work_size = 1;
